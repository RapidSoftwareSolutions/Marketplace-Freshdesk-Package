...
$app->post('/api/Freshdesk/updateComment', function ($request, $response) {
    ...
    $validateRes = $checkRequest->validate($request, ['apiKey', 'domain', 'commentId']);
    ...
    if (isset($postData['args']['answer']) && strlen($postData['args']['answer']) > 0) {
        $json['answer'] = filter_var($postData['args']['answer'], FILTER_VALIDATE_BOOLEAN);
    }
    if (isset($postData['args']['body']) && strlen($postData['args']['body']) > 0) {
