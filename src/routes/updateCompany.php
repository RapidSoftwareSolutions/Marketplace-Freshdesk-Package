...
$app->post('/api/Freshdesk/updateCompany', function ($request, $response) {
    ...
    $validateRes = $checkRequest->validate($request, ['apiKey', 'domain', 'companyId']);
    ...
    $url = "https://" . $postData['args']['domain'] . "." . $settings['apiUrl'] . "/companies/" . $postData['args']['companyId'];
    ...
    if (isset($postData['args']['domains']) && !empty($postData['args']['domains'])) {
        if (is_array($postData['args']['domains'])) {
            $json['domains'] = $postData['args']['domains'];
        } else {
            $json['domains'] = explode(',', $postData['args']['domains']);
        }
    }
