...
$app->post('/api/Freshdesk/getSingleTicket', function ($request, $response) {
    ...
    $included = [];
    $params = [];
    if (isset($postData['args']['includeConversations']) && filter_var($postData['args']['includeConversations'], FILTER_VALIDATE_BOOLEAN)) {
        $included[] = 'conversations';
    }
    if (isset($postData['args']['includeRequester']) && filter_var($postData['args']['includeRequester'], FILTER_VALIDATE_BOOLEAN)) {
        $included[] = 'requester';
    }
    if (isset($postData['args']['includeCompany']) && filter_var($postData['args']['includeCompany'], FILTER_VALIDATE_BOOLEAN)) {
        $included[] = 'company';
    }
    if (isset($postData['args']['includeStats']) && filter_var($postData['args']['includeStats'], FILTER_VALIDATE_BOOLEAN)) {
        $included[] = 'stats';
    }

    if (!empty($included)) {
        $params['include'] = implode(',', $included);
    }
